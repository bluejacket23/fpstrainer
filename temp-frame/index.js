...
// amplify/functions/frame-extractor/handler.ts
var handler_exports = {};
...
var AI_FUNCTION_NAME = process.env.AI_FUNCTION_NAME;
...
var handler = async (event) => {
  ...
  for (const record of event.Records) {
    ...
    const reportId = path.basename(parts[2], ".mp4");
    try {
      ...
      await (0, import_promises.pipeline)(response.Body, fs.createWriteStream(localInput));
      console.log("Video downloaded");
      if (fs.existsSync("/opt/bin/ffmpeg")) {
        import_fluent_ffmpeg.default.setFfmpegPath("/opt/bin/ffmpeg");
      }
      let videoDuration = 0;
      await new Promise((resolve, reject) => {
        import_fluent_ffmpeg.default.ffprobe(localInput, (err, metadata) => {
          if (err) {
      ...
      await new Promise((resolve, reject) => {
        (0, import_fluent_ffmpeg.default)(localInput).outputOptions([
          "-vf",
          "fps=1,scale=1280:720:force_original_aspect_ratio=decrease",
          "-q:v",
          "8",
          // JPEG quality (2-31, lower is better quality, 8 is good balance)
          "-threads",
          "4"
          // Use multiple threads for faster encoding
        ]).output(`${outputDir}/frame_%03d.jpg`).on("end", resolve).on("error", (err) => {
      ...
      if (files.length > 0) {
        ...
        const thumbnailCommand = new import_client_s3.GetObjectCommand({
          Bucket: bucketToUse,
          Key: thumbnailKey
        });
        thumbnailUrl = await (0, import_s3_request_presigner.getSignedUrl)(s3, thumbnailCommand, { expiresIn: 604800 });
      ...
      try {
        ...
        const invokeResponse = await lambda.send(new import_client_lambda.InvokeCommand({
          ...
          Payload: JSON.stringify({
            ...
            // Pass duration for accurate timestamps
          })
        }));
